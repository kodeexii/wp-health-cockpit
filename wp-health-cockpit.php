...
function matgem_register_settings() {
    ...
    add_settings_section(
        ...
        'matgem_settings_section_callback',
        'wp-health-cockpit'
    );
    add_settings_field(
        'whc_total_ram',
        'Jumlah RAM Server (GB)',
        'matgem_ram_field_callback',
        'wp-health-cockpit',
        'whc_settings_section'
    );
}
...
function matgem_add_admin_menu() {
    add_management_page(
        'WP Health Cockpit',      // Page title
        'Health Cockpit',         // Menu title
        'manage_options',         // Capability
        'wp-health-cockpit',      // Menu slug
        'matgem_render_audit_page' // Callback function
    );
}
...
function matgem_get_database_info() {
    ...
    $table_engines = $wpdb->get_results("SELECT table_name, engine FROM information_schema.TABLES WHERE table_schema = '" . DB_NAME . "' AND table_name IN ('" . implode("','", $core_tables) . "')");
    ...
    $buffer_pool_size_row = $wpdb->get_row("SHOW VARIABLES LIKE 'innodb_buffer_pool_size'", ARRAY_A);
    $buffer_pool_size_bytes = $buffer_pool_size_row ? $buffer_pool_size_row['Value'] : 0;
    $buffer_pool_size_mb = round($buffer_pool_size_bytes / 1024 / 1024);
    
    $rec_from_db_size = floor($total_db_size_mb * 1.2);
    $final_rec_mb = $rec_from_db_size;

    if ($total_ram_gb > 0) {
        $rec_from_ram = floor($total_ram_gb * 1024 * 0.25);
        if ($rec_from_ram < $final_rec_mb) {
            $final_rec_mb = $rec_from_ram;
        }
    }

    if ($final_rec_mb < 256) { $final_rec_mb = 256; }

    $status_buffer_pool = 'ok';
    if ($buffer_pool_size_mb < ($final_rec_mb * 0.75)) { $status_buffer_pool = 'warning'; }
    if ($buffer_pool_size_mb < ($final_rec_mb * 0.5)) { $status_buffer_pool = 'critical'; }
...
function matgem_render_audit_page() {
    ...
    $server_diag_data = function_exists('matgem_get_server_diagnostics') ? matgem_get_server_diagnostics() : [];
    ?>
    ...
    <div class="wrap">
        ...
        <?php if (!empty($server_diag_data)): ?>
            <h2 style="margin-top: 40px;">Diagnostik Server</h2>
            <table class="whc-table">
                <thead><tr><th>Perkara</th><th>Status Semasa</th><th>Cadangan</th><th>Status</th><th>Nota</th></tr></thead>
                <tbody><?php foreach ($server_diag_data as $data) : ?><tr><td><strong><?php echo esc_html($data['label']); ?></strong></td><td><?php echo esc_html($data['value']); ?></td><td><?php echo esc_html($data['recommended']); ?></td><td class="whc-status"><span class="<?php echo esc_attr('status-' . $data['status']); ?>"><?php echo esc_html($data['status']); ?></span></td><td><?php echo esc_html($data['notes']); ?></td></tr><?php endforeach; ?></tbody>
            </table>
        <?php endif; ?>
